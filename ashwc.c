...
static void focus_toplevel(struct ashwc_toplevel *toplevel) {
    ...
    struct ashwc_server *server = toplevel->server;
...
static void server_new_keyboard(struct ashwc_server *server,
...
static void server_new_input(struct wl_listener *listener, void *data) {
     ...
     * communiciated to the client. In ashwc we always have a cursor, even if
...
static void seat_request_cursor(struct wl_listener *listener, void *data) {
    struct ashwc_server *server = wl_container_of(
...
static void process_cursor_resize(struct ashwc_server *server) {
    /*
...
static void process_cursor_motion(struct ashwc_server *server, uint32_t time) {
...
static void xdg_toplevel_commit(struct wl_listener *listener, void *data) {
    ...
    if (toplevel->xdg_toplevel->base->initial_commit) {
         ...
         * reply with a configure so the client can map the surface.ashwc 
...
static void xdg_toplevel_destroy(struct wl_listener *listener, void *data) {
    ...
    struct ashwc_toplevel *toplevel = wl_container_of(listener, toplevel, destroy);
...
static void begin_interactive(struct ashwc_toplevel *toplevel,
        enum ashwc_cursor_mode mode, uint32_t edges) {
...
static void xdg_toplevel_request_move(
    ...
    struct ashwc_toplevel *toplevel = wl_container_of(listener, toplevel, request_move);
    begin_interactive(toplevel, ASHWC_CURSOR_MOVE, 0);
}
...
static void xdg_toplevel_request_resize(
    ...
    struct ashwc_toplevel *toplevel = wl_container_of(listener, toplevel, request_resize);
...
static void server_new_xdg_toplevel(struct wl_listener *listener, void *data) {
    ...
    struct ashwc_server *server = wl_container_of(listener, server, new_xdg_toplevel);
    struct wlr_xdg_toplevel *xdg_toplevel = data;

    /* Allocate a ashwc_toplevel for this surface */
...
static void xdg_popup_commit(struct wl_listener *listener, void *data) {
    ...
    if (popup->xdg_popup->base->initial_commit) {
         ...
         * ashwc sends an empty configure. A more sophisticated compositor
...
static void server_new_xdg_popup(struct wl_listener *listener, void *data) {
    struct ashwc_server *server = wl_container_of(listener, server, new_xdg_popup);
    struct wlr_xdg_popup *xdg_popup = data;

    // parent can be NULL or invalid in some edge cases
    if (!xdg_popup->parent) {
        return;
    }
    ...
    if (parent_xdg != NULL) {
        ...
        if (parent_layer != NULL) {
            struct ashwc_layer_surface *ls = parent_layer->data;
            parent_tree = ls->scene_layer->tree;
        } else {
            free(popup);
            return;
        }
    ...
    // guard against parent_tree having no data set yet
    if (!parent_tree) {
        free(popup);
        return;
    }

...
void
layout_grid(struct ashwc_workspace *workspace) {
    struct ashwc_output *output = workspace->output;

    uint32_t count =
        wl_list_length(&workspace->masters) +
        wl_list_length(&workspace->slaves);
...
#define PLACE_WINDOW(win) \
    do { \
        ...
        uint32_t x = output->usable_area.x + outer \
            + col * (cell_width + 2 * border + 2 * inner) \
            + border; \
        uint32_t y = output->usable_area.y + outer \
            + row * (cell_height + 2 * border + 2 * inner) \
            + border; \

...
void server_handle_new_layer_surface(struct wl_listener *listener, void *data) {
  ...
  layer_surface->something.type = ASHWC_LAYER_SURFACE;
  ...
  layer_surface->scene =
      wlr_scene_layer_surface_v1_create(scene, wlr_layer_surface);
...
void layer_surface_handle_map(struct wl_listener *listener, void *data) {
  ...
  struct ashwc_output *output = wlr_layer_surface->output->data;
  ...
  if (server.config->blur) {
    ...
    wl_list_for_each(b, &server.config->layer_rules.blur, link) {
      if (!b->condition.has ||
          regexec(&b->condition.regex, wlr_layer_surface->namespace, 0, NULL,
                  0) == 0) {
        wlr_scene_node_for_each_buffer(&layer_surface->scene->tree->node,
  ...
  wlr_scene_layer_surface_v1_configure(layer_surface->scene, &output_box,
                                       &output->usable_area);
  ...
  if (layer_surface == server.focused_layer_surface) {
    ...
    if (!focused) {
      ...
      if (server.prev_focused != NULL &&
        ...
        struct ashwc_toplevel *first =
            wl_container_of(server.active_workspace->masters.next, first, link);
        focus_toplevel(first);
      } else if (!wl_list_empty(&server.active_workspace->floating_toplevels)) {
        struct ashwc_toplevel *first = wl_container_of(
            server.active_workspace->floating_toplevels.next, first, link);
        focus_toplevel(first);
      }
    }
  }
...
void layer_surface_handle_new_popup(struct wl_listener *listener, void *data) {
  ...
  struct ashwc_popup *popup = xdg_popup->base->data;
...
void focus_layer_surface(struct ashwc_layer_surface *layer_surface) {
  if (server.lock != NULL)
    return;
  ...
  server.exclusive = keyboard_interactive ==
                     ZWLR_LAYER_SURFACE_V1_KEYBOARD_INTERACTIVITY_EXCLUSIVE;
  struct wlr_keyboard *keyboard = wlr_seat_get_keyboard(server.seat);
  if (keyboard != NULL) {
    wlr_seat_keyboard_notify_enter(
        server.seat, layer_surface->wlr_layer_surface->surface,
        keyboard->keycodes, keyboard->num_keycodes, &keyboard->modifiers);
  }
}

void layer_surfaces_commit_layer(struct ashwc_output *output,
                                 enum zwlr_layer_shell_v1_layer layer,
                                 bool exclusive) {
  struct wl_list *list = layer_get_list(output, layer);
  ...
  wl_list_for_each_reverse(l, list, link) {
    ...
    wlr_scene_layer_surface_v1_configure(l->scene, &full_area,
                                         &output->usable_area);
  }
}

void layer_surfaces_commit(struct ashwc_output *output) {
  struct wlr_box full_area;
  wlr_output_layout_get_box(server.output_layout, output->wlr_output,
                            &full_area);
...
struct wlr_scene_tree *layer_get_scene(enum zwlr_layer_shell_v1_layer layer) {
  switch (layer) {
...
struct wl_list *layer_get_list(struct ashwc_output *output,
                               enum zwlr_layer_shell_v1_layer layer) {
  switch (layer) {
  case ZWLR_LAYER_SHELL_V1_LAYER_BACKGROUND:
 ...
 */
void layers_under_fullscreen_set_enabled(struct ashwc_output *output,
                                         bool enable) {
  struct ashwc_layer_surface *l;

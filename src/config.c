...
bool parse_color_rgba_or_hex(char **args, size_t arg_count, float *dest) {
  if (arg_count == 4) {
...
struct vec2 calculate_animation_curve_at(struct ashwc_config *c, double t) {
  ...
  point.x = 3 * t * (1 - t) * (1 - t) * c->animation_curve[0] +
            3 * t * t * (1 - t) * c->animation_curve[2] + t * t * t;

  point.y = 3 * t * (1 - t) * (1 - t) * c->animation_curve[1] +
            3 * t * t * (1 - t) * c->animation_curve[3] + t * t * t;
...
void bake_bezier_curve_points(struct ashwc_config *c) {
  ...
  for (size_t i = 0; i < BAKED_POINTS_COUNT; i++) {
    c->baked_points[i] =
        calculate_animation_curve_at(c, (double)i / (BAKED_POINTS_COUNT - 1));
  }
}

bool config_add_layer_rule(struct ashwc_config *c, char *regex, char *predicate,
...
bool config_add_window_rule(struct ashwc_config *c, char *app_id_regex,
  ...
  if (strcmp(predicate, "float") == 0) {
    ...
    window_rule->inactive_value = arg_count > 1 ? clamp(atof(args[1]), 0.0, 1.0)
...
char *string_append_with_comma(char *a, char *b, size_t *cap, bool comma) {
  ...
  /* now there is enough space to fit the new one; we add , if its not the first
   * one */
  if (comma) {
    a[a_len] = ',';
    a_len++;
...
void config_add_keymap(struct ashwc_config *c, char *layout, char *variant) {
  ...
  c->keymap_layouts =
      string_append_with_comma(c->keymap_layouts, layout, &layout_cap, count);
  c->keymap_variants = string_append_with_comma(c->keymap_variants, variant,
                                                &variant_cap, count);
...
bool config_handle_value(struct ashwc_config *c, char *keyword, char **args,
  ...
  if (strcmp(keyword, "min_toplevel_size") == 0) {
    ...
    enum libinput_config_accel_profile accel =
        atoi(args[1]) ? LIBINPUT_CONFIG_ACCEL_PROFILE_ADAPTIVE
                      : LIBINPUT_CONFIG_ACCEL_PROFILE_FLAT;
  ...
  } else if (strcmp(keyword, "natural_scroll") ==
    ...
    *w = (struct workspace_config){
    ...
    config_add_window_rule(c, args[0], args[1], args[2], &args[3],
                           arg_count - 3);
    ...
    wlr_log(WLR_ERROR, "placeholder_color has been depricated, and should not "
                       "be used anymore");
    goto depricated;
...
void get_default_config_path(char *dest, size_t size) {
  ...
  if (default_config_path == NULL) {
    ...
    wlr_log(WLR_INFO,
            "no env ASHWC_DEFAULT_CONFIG_PATH set, using the default %s",
            default_config_path);
  } else {
    wlr_log(WLR_INFO, "env ASHWC_DEFAULT_CONFIG_PATH set to %s, using it",
            default_config_path);
  }
...
bool get_config_path(char *dest, size_t size) {
  char *env_conf = getenv("ASHWC_CONFIG_PATH");
  if (env_conf != NULL) {
    snprintf(dest, size, "%s", env_conf);
    return true;
  }
...
bool config_handle_line(char *line, size_t line_number, char **keyword,
  ...
  *q = 0;
  ...
  if (*p == '\n') {
    wlr_log(WLR_ERROR, "config: line %zu: no args provided for %s", line_number,
            kw);
    return false;
  }
...
void config_set_default_needed_params(struct ashwc_config *c) {
  ...
  if (c->keyboard_delay == 0) {
    c->keyboard_delay = 50;
    wlr_log(WLR_INFO, "keyboard_delay not specified. using default %ud",
            c->keyboard_delay);
  }
  if (c->cursor_size == 0) {
    c->cursor_size = 24;
    wlr_log(WLR_INFO, "cursor_size not specified. using default %ud",
            c->cursor_size);
  }
  if (c->min_toplevel_size == 0) {
    c->min_toplevel_size = 10;
    wlr_log(WLR_INFO, "min_toplevel_size not specified. using default %ud",
            c->min_toplevel_size);
  }
  if (c->master_count == 0) {
    c->master_count = 1;
    wlr_log(WLR_INFO, "master_count not specified. using default %lf",
            c->master_ratio);
  }
  if (c->master_ratio == 0) {
  ...
  if (c->animations && c->animation_duration == 0) {
    ...
    wlr_log(WLR_INFO, "animation_duration not specified. using default %ud",
            c->animation_duration);
  }
  if (c->animations && c->animation_curve[0] == 0 &&
      c->animation_curve[1] == 0 && c->animation_curve[2] == 0 &&
      c->animation_curve[3] == 0) {
    bake_bezier_curve_points(c);
    wlr_log(WLR_INFO, "animation_curve not specified. baking default linear");
  }
  if (c->inactive_opacity == 0) {
    c->inactive_opacity = 1.0;
    wlr_log(WLR_INFO, "inactive_opacity not specified. using default %lf",
            c->inactive_opacity);
  }
...
static bool config_parse_file(struct ashwc_config *c, const char *path) {
  FILE *config_file = fopen(path, "r");
  if (config_file == NULL) {
    wlr_log(WLR_ERROR, "Couldn't open config file: %s", path);
    return false;
  }
  ...
  while (fgets(line_buffer, sizeof(line_buffer), config_file)) {
    if (!config_handle_line(line_buffer, line_number, &keyword, &args,
                            &args_count)) {
      line_number++;
      continue;
    }
    ...
    if (strcmp(keyword, "import") == 0) {
      ...
      if (strcmp(keyword, "import") == 0) {
        if (args_count > 0) {
          ...
          if (args[0][0] == '~') {
            snprintf(import_path, sizeof(import_path), "%s%s", getenv("HOME"),
                     args[0] + 1);
          } else {
            strncpy(import_path, args[0], sizeof(import_path) - 1);
            import_path[sizeof(import_path) - 1] = '\0';
          }

          config_parse_file(c, import_path);
        } else {
          wlr_log(WLR_ERROR, "import requires a path");
        }
      } else {
...
struct ashwc_config *config_load(void) {
  ...
  if (get_config_path(path, sizeof(path))) {
    ...
    if (test != NULL) {
      ...
      wlr_log(WLR_INFO, "couldn't open config file, using default");
      get_default_config_path(path, sizeof(path));
    }
  } else {
    wlr_log(WLR_INFO, "couldn't get config path, using default");
    get_default_config_path(path, sizeof(path));
  }
...
void config_destroy(struct ashwc_config *c) {
  ...
  wl_list_for_each_safe(k, k_temp, &c->pointer_keybinds, link) { free(k); }
...
void toplevel_reapply_effects_etc(struct ashwc_toplevel *toplevel) {
...
void layout_reorganize(struct ashwc_workspace *workspace) {
...
void config_reload() {
  ...
  if (c->blur) {
    ...
    wl_list_for_each(output, &server.outputs, link) {
      ...
      output->blur = wlr_scene_optimized_blur_create(
          &server.scene->tree, output_box.width, output_box.height);
      wlr_scene_set_blur_data(
          server.scene, server.config->blur_params.num_passes,
          server.config->blur_params.radius, server.config->blur_params.noise,
  ...
  wl_list_for_each(out, &server.outputs, link) {
    ...
    wl_list_for_each(w, &out->workspaces, link) {
      ...
      wl_list_for_each(t, &w->slaves, link) { toplevel_reapply_effects_etc(t); }
    ...
    for (size_t i = 0; i < 4; i++) {
      wl_list_for_each(layer, &(&out->layers.background)[i], link) {
        ...
        wl_list_for_each(b, &server.config->layer_rules.blur, link) {
          if (!b->condition.has ||
              regexec(&b->condition.regex, layer->wlr_layer_surface->namespace,
                      0, NULL, 0) == 0) {
            wlr_scene_node_for_each_buffer(&layer->scene->tree->node,
                                           iter_scene_buffer_apply_blur,
                                           (void *)1);
            found = true;
            break;
          }
...
void idle_reload_config(void *data) {
...
void *config_watch(void *arg) {
  ...
  while (1) {
    ...
    for (char *ptr = buffer; ptr < buffer + length;
         ptr += EVENT_SIZE + ((struct inotify_event *)ptr)->len) {

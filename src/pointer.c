...
bool pointer_configure(struct ashwc_pointer *pointer) {
  ...
  if (libinput_device_config_accel_is_available(device)) {
    if (libinput_device_config_accel_set_speed(device, sensitivity) !=
      ...
      wlr_log(WLR_ERROR, "applying sensitivity to device '%s' failed",
              pointer->name);
    }

    if (accel) {
      struct libinput_config_accel *accel_config =
          libinput_config_accel_create(accel);
      if (libinput_device_config_accel_apply(device, accel_config) !=
          LIBINPUT_CONFIG_STATUS_SUCCESS) {
        wlr_log(WLR_ERROR,
                "applying acceleration profile to device '%s' failed",
                pointer->name);
      }
  ...
  if (libinput_device_config_tap_get_finger_count(device) > 0) {
    ...
    if (libinput_device_config_tap_set_enabled(
            device, server.config->trackpad_tap_to_click) !=
        LIBINPUT_CONFIG_STATUS_SUCCESS) {
      wlr_log(WLR_ERROR, "applying tap to click to device '%s' failed",
              pointer->name);
    }

    if (libinput_device_config_scroll_set_natural_scroll_enabled(
            device, server.config->trackpad_natural_scroll) !=
        LIBINPUT_CONFIG_STATUS_SUCCESS) {
      wlr_log(WLR_ERROR, "applying natural scroll to device '%s' failed",
              pointer->name);
    }

    if (libinput_device_config_scroll_set_method(
            device, server.config->trackpad_scroll_method) !=
        LIBINPUT_CONFIG_STATUS_SUCCESS) {
      wlr_log(WLR_ERROR, "applying scroll method to device '%s' failed",
              pointer->name);
    }

    if (libinput_device_config_dwt_set_enabled(
            device, server.config->trackpad_disable_while_typing) !=
        LIBINPUT_CONFIG_STATUS_SUCCESS) {
      wlr_log(WLR_ERROR, "applying disable while typing to device '%s' failed",
              pointer->name);
    }
  }
...
void cursor_handle_motion(uint32_t time) {
  ...
  /* set global active workspace and stop moving resizing if there is a
    ...
    if (output->active_workspace->fullscreen_toplevel != NULL) {
      if (server.cursor_mode == ASHWC_CURSOR_MOVE) {
        if (!server.grabbed_toplevel->floating) {
          ...
          wl_list_insert(&prev_workspace->floating_toplevels,
...
void server_handle_cursor_button(struct wl_listener *listener, void *data) {
  ...
  if (event->state == WL_POINTER_BUTTON_STATE_RELEASED &&
    ...
    if (!server.grabbed_toplevel->floating) {
      ...
      wl_list_insert(server.active_workspace->floating_toplevels.next,
                     &server.grabbed_toplevel->link);
    }
...
void constraint_remove_current(void) {
  ...
  wlr_pointer_constraint_v1_send_deactivated(
      server.current_constraint->wlr_pointer_constraint);
}
...
void constraint_move_to_hint(struct ashwc_pointer_constraint *constraint) {
  struct wlr_pointer_constraint_v1 *wlr_constraint =
      constraint->wlr_pointer_constraint;
  ...
  if (wlr_constraint->current.committed &
    ...
    wlr_cursor_warp(server.cursor, NULL, X(server.focused_toplevel) + sx,
                    Y(server.focused_toplevel) + sy);

    /* make sure we are not sending unnecessary surface movements (took from
     * labwc)*/
...
void constraint_handle_destroy(struct wl_listener *listener, void *data) {
  struct ashwc_pointer_constraint *constraint =
...
void constrain_apply_to_move(double *dx, double *dy) {
  ...
  if (server.current_constraint->wlr_pointer_constraint->type ==
  ...
  if (wlr_region_confine(
          &server.current_constraint->wlr_pointer_constraint->region, current_x,
          current_y, current_x + *dx, current_y + *dy, &constrained_x,
          &constrained_y)) {
